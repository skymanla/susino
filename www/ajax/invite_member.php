...
if($sb_id == $sbi_option){
	...
	if($r['cnt'] == "1"){
		...
		if($conn->query($sql)){
			if(!empty($sbi_prize)){
				$content = $sb_id."님! 스시노백쉐프 우리, 함께 갈래요? 이벤트에 당첨되신 것을 축하드립니다! ".$sb_id."님의 초대에 응답해주신 함께 가고 싶은 분과
							스시노백쉐프에서 맛있는 초밥을 드셔보세요! 스시노백쉐프 키프트카드 3만원권은 다음날 발송될 예정입니다. 기프트카드의 유효기간과, 사용가능 매장 확인 후 방문 부칵드립니다!";
			}
