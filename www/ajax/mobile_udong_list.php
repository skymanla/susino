...
$area_where = " ( sbab_area like '%".$dash_row['dongnae']."%' or sbab_area='A' ) and (sbab_lvl='' or sbab_lvl='".$sb_level."' or sbab_lvl='A') ";
		...
		<ul class="bo_webzine1">
			...
			if($v->num_rows == 0){
				...
				foreach($v as $key => $event_board){
					...
					$link_url = "/m/page/s5/s2sview.php?idx=".$event_board['sbab_idx']."&page=".$cur_page."&type=ourdongnae";
					if($now_date > $edate){
						$app_stat = 'end';
						$app_stat_title = '마감';
