	...
	<table>
		...
		<tbody>
			...
			foreach($query as $key => $row){
				...
				//글 제목
				$sql = "select sbab_cate, sbab_title from sb_application_board where sbab_idx='".$row['sbabm_fidx']."'";
				$q = $conn->query($sql);
				$title = $q->fetch_assoc();
				switch($title['sbab_cate']){
					case "shopper":
						$event_cate = "미스테리 쇼퍼";
						break;
					case "ftalk":
				...
				//회원 정보
				$sql = "select * from sb_member a join sb_member_level b on a.sb_mem_level=b.sb_level_cate where sb_id='".$row['sbr_wid']."'";
			?>
			<tr>
				<td class="txt_c"><?=$board_no?></td>
				<td class="txt_c"><?=$event_stat?></td>
				<td class="txt_c"><?=date('Y-m-d', strtotime($row['sbr_rdate']))?></td>
				<td class="txt_c"><?=$event_cate=='' ? "자발적 참여" : $event_cate?></td>
			...
			<? 
				...
				echo '<tr><td colspan="11" align=center>등록된 후기가 없습니다.</td></tr>';
			}
			?>
		</tbody>
	</table>
