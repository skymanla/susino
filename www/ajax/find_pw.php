...
if(!$cnt['cnt']=='0'){
	$charactersLength = strlen($sb_find_id);
	$randomString = "";
	for($i=0;$i<10;$i++){
		$randomString .= $sb_find_id[rand(0, $charactersLength-1)];
	}
	$chg_tmp_pwd = password_hash($randomString, PASSWORD_BCRYPT);
	$sql = "update sb_member set sb_password='".$chg_tmp_pwd."' where sb_id='".$sb_find_id."'";
	if($conn->query($sql)){
