					...
					$.each(a, function(i,item){
						if(i < n){
							...
							switch(type)
							{
								...
								if(cp.length > 1){
									...
									if(o.comments > 0 && item.comments){
										i = 0;
										m += '<div class="meta"><div class="comments">comments</div></div>';
										$.each(item.comments.data, function(i,cmt){
											if(o.comments > i){
												m += '<div class="meta item-comments"><a href="http://facebook.com/'+cmt.from.id+'">'+cmt.from.name+'</a>'+cmt.message+'</div>';
												i++;
											} else {
												return false;
											}
								...
								z = '<div class="twitter-user"><a href="https://www.twitter.com/'+un+'"><strong>'+item.user.name+' </strong>@'+un+'</a></div>';
								...
								x = '<a href="'+q+'">'+st+'</a>';
								z = '<div class="meta"><div class="views">'+num(dribbble.views_count)+'</div><div class="likes">'+num(dribbble.likes_count)+'</div><div class="comments">'+num(dribbble.comments_count)+'</div></div>';
								sq = q;
								break;
								...
								if(o.stats == true){
									var m = '';
									m += f == 'albums' || f == 'channels' || f == 'groups' ? '<div class="videos">'+num(item.total_videos)+'</div>' : '' ;
									if(f == 'channels'){
										m += '<div class="users">'+num(item.total_subscribers)+'</div>';
									} else if(f == 'groups'){
										m += '<div class="users">'+num(item.total_members)+'</div>';
									} else if(f != 'albums'){
										m += '<div class="likes">'+num(item.stats_number_of_likes)+'</div><div class="views">'+num(item.stats_number_of_plays)+'</div><div class="comments">'+num(item.stats_number_of_comments)+'</div>';
									}
									z += '<div class="meta">'+m+'</div>';
								}
								...
								if(o.shares){
									z = '<div class="meta"><div class="plusones">+1s '+m+'</div><div class="shares">'+p+'</div><div class="comments">'+g+'</div></div>';
								}
								sq = q;
								st = dl.useTitle;
								break;
							...
							$.each(o.out.split(','), function(i,v){
								...
								zz += v != 'intro' ? '<div class="section-'+v+'">' : '' ;
								switch(v)
								{
									case 'intro':
									if(type == 'twitter'){
										zintro = '<div class="section-'+v+'"><a href="'+q+'">'+decodeURIComponent(intro)+'</a> <div><a href="https://twitter.com/'+un+'/status/'+item.id_str+'">'+nicetime(new Date(d).getTime(),0)+'</a></div></div>';
									} else {
										zintro = '<div class="section-'+v+'"><a href="'+q+'">'+decodeURIComponent(intro)+'</a> <div>'+nicetime(new Date(d).getTime(),0)+'</div></div>';
									}
									break;
									case 'title':
									zz += x;
							...
							windD = '<div class="section-intro"><div class="section_top_bg"><table class="bg_table" border="0" cellpadding="0" cellspacing="0"><tr><td class="left_bg"></td><td class="line_bg"></td><td class="right_bg"></td></tr></table></div><div>'+nicetime(new Date(d).getTime(),0)+'</div></div>';
							var out = '<li rel="'+ob+'" class="dcsns-li dcsns-'+type+' dcsns-feed-'+fn+'">'+w+'<div class="inner">'+windD+icon+'<div class="clear"></div>'+zz+'<div class="clear"></div></div><div class="section_bottom_bg"><table class="bg_table" border="0" cellpadding="0" cellspacing="0"><tr><td class="left_bg"></td><td class="line_bg"></td><td class="right_bg"></td></tr></table></div></li>', str = decodeURIComponent(opt.remove), rem = q;
							...
							if( str.indexOf( rem ) !== -1 && rem != '' ){
								...
								var wPush = true;
								var wZ = z.split('#');
								var noHash = opt.noHash.split(',');
								$.each(wZ,function (i, k){
									$.each(noHash,function (i, j){
										if(k.trim().indexOf(j.split('#')[1]) != -1){
											wPush = false;
											return false;
										}
									});
								...
								var wU = u.replace(/(<([^>]+)>)/ig,"");
								var noUser = opt.noUser.split(',');
								$.each(noUser,function (i, j){
									if(wU === j.trim()){
										wPush = false;
										return false;
									}
	...
	function nicetime(a,out){
        ...
        if(out == 1) {
            ...
            var chunks = new Array();
                    chunks[0] = [60 * 60 * 24 * 365 , '년', '년'];
                    chunks[1] = [60 * 60 * 24 * 30 , '달', '달'];
                    chunks[2] = [60 * 60 * 24 * 7, '주', '주'];
                    chunks[3] = [60 * 60 * 24 , '일', '일'];
                    chunks[4] = [60 * 60 , '시간', '시간'];
                    chunks[5] = [60 , '분', '분'];
