...
$sql_notice = "select * from sb_application_notice_board where (sbab_area like'%".$dongnae[1]."%' or sbab_area='A') order by sbab_idx limit 0, 10";
		...
		<div class="my_notice_roll_wrap">
			...
			<div class="roll_wrap swiper-container-notice slide-wrap">
				<ul class="roll swiper-wrapper slide">					
					...
					?>
					<li class="swiper-slide"><a href="/m/page/s5/s2snotice_view.php?idx=<?=$notice['sbab_idx']?>"><?=$notice['sbab_title']?></a></li>
					<? 
						}
					}
					?>
...
function tab1_list(getPage, getType1, getType2){
	$.ajax({
		type : "POST",
		url : "/ajax/mobile_udong_list.php",
