...
//개수
$count = "SELECT COUNT(sbb_idx) as cnt FROM $tbl_info";
...
$query = "SELECT * FROM $tbl_info ORDER BY sbb_rdate DESC LIMIT $limit_num OFFSET $show_offset_num";
		...
		<table>
			...
			<tbody>
				...
				<tr>
					...
					<td class="txt_c"><a href="./s3sview.php?idx=<?php echo $row['sbb_idx'];?>"><?php echo $sbb_aria;?></a></td>
					<td class="txt_c"><?=$row[sbb_use]=='99' ? "10년 이상" : $row[sbb_use]."년"?></td>
					<td class="txt_c"><?php echo $row['sbb_name'];?></td>
					<td class="txt_c"><?php echo $row['sbb_hp'];?></td>
					<td class="txt_c"><?php echo $rdate; ?></td>
				</tr>
				<?php
					$i++;
					$board_no--;
		...
		function modiy_stat(mode){
			if(mode=="D"){
			    ...
			    $.ajax({
			    	...
			    	data : {"mode": mode, "pageinfo" : "s1_s3", "chk_idx" : chk_data},
