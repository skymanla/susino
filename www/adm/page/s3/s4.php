...
switch($_GET['mvpage']){
	case "ing" :
		...
		$where[] = ' "'.date('Y-m-d').'"  between date_format(sbab_sdate, "%Y-%m-%d") and date_format(sbab_edate, "%Y-%m-%d")';
		break;
	case "endding" :
...
<section class="section1">
		...
		<table>
			...
			<tbody>
				...
				<tr>
					...
					<td class="txt_c"><a href="s4sview.php?idx=<?=$r[sbab_idx]?>" class="bt_s1">자세히보기</a></td>
				</tr>
				<? 
					$board_no--;
					}
	...
	<div class="bt_wrap1">
		...
		<div class="right_box">
			<a href="s4swrite.php" class="bt_1">체험단 등록</a>
		</div>
	</div>
	...
	<script>
		function mv_page(getVal){
			if(getVal == "a"){
				...
				location.href = './s4.php?mvpage=ing';
			}else if(getVal == "endding"){
				location.href = './s4.php?mvpage=endding';
			}
		}
